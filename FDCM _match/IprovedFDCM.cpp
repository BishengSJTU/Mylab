...
void  IprovedFDCM::showMatch(Mat distmap, Vec4f matchline, Vec4f curQline, int minD, vector<Vec4f> Qlines, vector<Vec4f> lines)
{
	...
	for (int i = 0; i < Qlines.size(); i++)
	{
		...
		line(distmap, pt1, pt2, CV_RGB(20, 250, 250), 2);
	}
	pt1.x = matchline[0];	pt1.y = matchline[1];	pt2.x = matchline[2];	pt2.y = matchline[3];
//	line(distmap, pt1, pt2, CV_RGB(0, 0, 255), 2);
...
double IprovedFDCM::ondistTrans(Mat src, vector<FastLenOrder>Qorder, vector<Vec4f> Qlines, vector<FastLenOrder>order, vector<Vec4f> lines,bool shows, double* rst) {   //距离变换
    rst[0] = 0;
    int rows = src.rows;
    int cols = src.cols;
    ...
    for (int i = 0; i < order.size(); i++) {  //每条直线都剪短，以防交叉
    ...
    if (Qlines.size() && lines.size()) {
        ...
        if (shows) {
            ...
            fpsSt += fpsC;
            fpsSt += "   distType";
            std::sprintf(fpsC, "%d", (int) distType);
            fpsSt += fpsC;
            fpsSt += "   maskSize";
            std::sprintf(fpsC, "%d", (int) maskSize);
            fpsSt += fpsC;
